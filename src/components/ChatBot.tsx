...
export function ChatBot({ isOpen, onClose }: ChatBotProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content: "こんにちは！RapidSaaSのAIアシスタントです。あなたのアプリ開発のお手伝いをします。まずは要件を伺っていきますね。",
      sender: "bot",
      timestamp: new Date(),
    },
  ]);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-4 right-4 w-[90%] sm:w-[450px] h-[600px] bg-card rounded-lg shadow-xl border border-border flex flex-col z-50"
          >
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-semibold text-foreground">RapidSaaS 要件定義アシスタント</h3>
              <Button
                variant="ghost"
                size="icon"
                onClick={onClose}
